...
import {
  bindInitialNavigationResourceSettle,
  getLayoutShiftResourcesBeforeEntry,
  getUnsettledLayoutShiftResourcesBeforeEntry,
  primeInitialNavigationResources,
} from "./initial-navigation-resources";
...
export function createInitialNavigationController({
  ...
  function primeLinkActivation(entry: TocEntry): void {
    primeInitialNavigationResources(
      getUnsettledLayoutShiftResourcesBeforeEntry(getState().article, entry),
    );
  }
    ...
    const sample = (): void => {
      ...
      if (
        ...
        startInitialNavigationLayoutWait(
          entry,
          getLayoutShiftResourcesBeforeEntry(getState().article, entry),
          { hasObservedLayoutShift: true },
        );
        return;
      }
  ...
  function waitForInitialNavigationLayout(entry: TocEntry): void {
    const resources = getLayoutShiftResourcesBeforeEntry(
      getState().article,
      entry,
    );
    if (resources.length === 0) return;

    const unsettledResources = getUnsettledLayoutShiftResourcesBeforeEntry(
      getState().article,
      entry,
    );
  ...
  function startLinkActivationWarmup(
    ...
    primeInitialNavigationResources(resources);
