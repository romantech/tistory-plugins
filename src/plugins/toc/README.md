...
- 제목에 `id`가 없으면 텍스트 기반 slug에 `rp-` prefix를 붙인 ID를 생성하고, 중복 시 고유한 ID로 보정합니다.
...
5. 대상 제목 앞에 로딩 중인 리소스가 있으면 짧게 대기한 뒤 스크롤합니다.
...
- 플러그인이 생성한 제목 ID는 스킨의 기존 `#pagination` 같은 스타일 ID와 충돌하지 않도록 `rp-` prefix를 사용합니다.
