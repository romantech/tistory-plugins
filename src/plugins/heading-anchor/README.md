...
- 제목에 `id`가 없으면 텍스트 기반 `slug`에 `rp-` prefix를 붙인 ID를 만들고, 이미 있더라도 중복되면 고유한 ID로 보정합니다.
...
`window.RPPlugins.headingAnchor` 속성을 통해 처리할 제목 레벨과 해시 이동 오프셋을 조정할 수 있습니다. 설정 코드는 스크립트보다 먼저 선언해야 합니다.
...
- 초기 해시 보정은 로드 직후 몇 차례 나눠 수행하므로, 환경에 따라 아주 짧은 추가 스크롤이 보일 수 있습니다.
